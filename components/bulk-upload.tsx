...
import { parseCSV, parseDMY, downloadCSV, TX_TYPES, CATEGORIES, CURRENCIES, TX_TYPE_LABELS } from '@/lib/utils'
import type { BulkRow, Category, TransactionType, Currency } from '@/lib/types'

// ── Templates ──────────────────────────────────────────────
const TX_HEADERS  = ['fecha','operacion','tipo','activo','cantidad','divisa','precio_unitario','total','tipo_cambio','notas']
const TX_EXAMPLES = [
  ['15/01/2024','buy','Bolsa','AAPL','10','USD','185.50','1855.00','1','Compra mensual DCA'],
  ['20/01/2024','deposit','Cash','Finsus','','MXN','','50000.00','17.50','Depósito enero'],
  ['01/02/2024','dividend','Bolsa','AAPL','','USD','','42.30','1','Dividendo Q1'],
]
...
function validateRow(raw: Record<string, string>, i: number): BulkRow {
  ...
  const tcRaw     = raw.tipo_cambio?.trim() ?? ''
  ...
  if (divisa === 'MXN' && (!tcRaw || isNaN(parseFloat(tcRaw)))) {
    errors.push('tipo_cambio es requerido cuando divisa es MXN (ej: 17.50)')
  }
  ...
  const tc    = divisa === 'USD' ? 1 : parseFloat(tcRaw || '1')
  ...
  return {
    ...
    parsed: errors.length === 0 ? {
      ...
      precio_unitario: price, total, tipo_cambio: tc,
...
export function BulkUpload({ mode, onDone }: { mode: Mode; onDone: () => void }) {
  ...
  function downloadTemplate() {
    if (mode === 'movimientos') downloadCSV('plantilla_movimientos.csv', TX_HEADERS, TX_EXAMPLES)
    else downloadCSV('plantilla_networth.csv', NW_HEADERS, NW_EXAMPLES)
  }
  ...
  async function importMovimientos() {
    ...
    for (const row of validRows) {
      ...
      try {
        ...
        const exchangeRate = p.tipo_cambio
  ...
  const s = {
    ...
    th:        { padding:'8px 12px', textAlign:'left' as const, fontSize:'11px', color:'var(--text-faint)', fontWeight:'400', textTransform:'uppercase' as const, letterSpacing:'0.06em', borderBottom:'1px solid var(--border)', whiteSpace:'nowrap' as const },
    td:        (color = 'var(--text-lo)') => ({ padding:'8px 12px', fontSize:'11px', color, borderBottom:'1px solid var(--border-sub)' } as React.CSSProperties),
    tdMono:    (color = 'var(--text-lo)') => ({ padding:'8px 12px', fontSize:'11px', color, fontFamily:"'JetBrains Mono', monospace", borderBottom:'1px solid var(--border-sub)', whiteSpace:'nowrap' as const } as React.CSSProperties),
  }
      ...
      {state === 'idle' && (
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'20px' }}>
            ...
            <p style={{ fontSize:'11px', color:'var(--text-faint)' }}>Descarga, llena y sube el mismo archivo</p>
          </div>
          ...
          >
            ...
            <p style={{ fontSize:'13px', color:'var(--text-lo)', marginBottom:'6px' }}>Arrastra tu .csv aquí</p>
          ...
          <div style={{ marginTop:'16px', padding:'14px 16px', background:'var(--bg-subtle)', border:'1px solid var(--border)' }}>
            <p style={{ fontSize:'10px', color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:'8px' }}>Columnas esperadas</p>
            {mode === 'movimientos' ? (
              <div style={{ display:'flex', flexWrap:'wrap', gap:'8px' }}>
                {[
                  { name:'fecha',           req:true,  note:'DD/MM/YYYY' },
                  { name:'operacion',       req:true,  note:'buy, sell, dividend, deposit, withdrawal, interest' },
                  { name:'tipo',            req:true,  note:'Negocio, Cash, Deuda, Bolsa' },
                  { name:'activo',          req:true,  note:'AAPL, Finsus, etc.' },
                  ...
                  { name:'tipo_cambio',     req:true,  note:'requerido si divisa=MXN (ej: 17.50), pon 1 si es USD' },
                  { name:'notas',           req:false, note:'opcional' },
                ].map(col => (
                  <div key={col.name} style={{ display:'flex', alignItems:'center', gap:'4px' }}>
                    <span style={{ fontFamily:"'JetBrains Mono', monospace", fontSize:'11px', color: col.req ? 'var(--text-hi)' : 'var(--text-muted)' }}>{col.name}</span>
      ...
      {state === 'preview' && (
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:'16px', marginBottom:'20px' }}>
            <span style={s.pill('var(--green)')}>{okCount} válidas</span>
            ...
            <button onClick={mode === 'movimientos' ? importMovimientos : importNetWorth}
          ...
          {mode === 'movimientos' && (
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead>
                  <tr>
                    {['#','Estado','Fecha','Operación','Tipo','Activo','Cantidad','Divisa','Total','TC','Notas'].map(h => (
                ...
                <tbody>
                  {rows.map(row => (
                    <tr key={row.lineNumber} style={{ background: row.status==='error' ? 'rgba(248,113,113,0.04)' : 'transparent' }}>
                      <td style={s.tdMono('var(--text-faint)')}>{row.lineNumber}</td>
                      <td style={{ padding:'8px 12px', borderBottom:'1px solid var(--border-sub)', minWidth:'100px' }}>
                        {row.status === 'ok'
                      ...
                      <td style={s.tdMono()}>{row.raw.fecha}</td>
                      <td style={s.td()}>{TX_TYPE_LABELS[row.raw.operacion?.toLowerCase()] ?? row.raw.operacion}</td>
                      <td style={s.td()}>{row.raw.tipo}</td>
                      <td style={s.td('var(--text-hi)')}>{row.raw.activo}</td>
                      <td style={s.tdMono('var(--text-muted)')}>{row.raw.cantidad || '—'}</td>
                      <td style={s.td('var(--text-muted)')}>{row.raw.divisa}</td>
                      <td style={s.tdMono('var(--text-hi)')}>{row.raw.total}</td>
                      <td style={s.tdMono('var(--text-muted)')}>{row.raw.tipo_cambio || '1'}</td>
                      <td style={{ ...s.td('var(--text-muted)'), maxWidth:'120px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{row.raw.notas || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {mode === 'networth' && (
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr>{['#','Estado','Fecha','Valor (USD)','Notas'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {nwRows.map(row => (
                  <tr key={row.lineNumber} style={{ background: row.error ? 'rgba(248,113,113,0.04)' : 'transparent' }}>
                    <td style={s.tdMono('var(--text-faint)')}>{row.lineNumber}</td>
                    <td style={{ padding:'8px 12px', borderBottom:'1px solid var(--border-sub)' }}>
                      {row.error
                        ? <div><span style={s.pill('var(--red)')}>Error</span><p style={{ fontSize:'10px', color:'var(--red)', marginTop:'3px' }}>{row.error}</p></div>
                        : <span style={s.pill('var(--green)')}>OK</span>}
                    </td>
                    <td style={s.tdMono()}>{row.fecha}</td>
                    <td style={s.tdMono('var(--text-hi)')}>${row.valor?.toLocaleString('es-MX', { minimumFractionDigits:2 })}</td>
                    <td style={s.td('var(--text-muted)')}>{row.notas || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
