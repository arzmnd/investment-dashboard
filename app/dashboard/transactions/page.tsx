...
const emptyTx  = { asset_id:'', transaction_type:'buy', quantity:'', price_per_unit:'', total_amount:'', fees:'0', notes:'', transaction_date: new Date().toISOString().split('T')[0] }
...
export default function TransactionsPage() {
  ...
  async function load() {
    ...
    const [{ data: transactions }, { data: assetList }, { data: snapshots }] = await Promise.all([
      supabase.from('transactions').select('*, asset:assets(name, ticker, category)').eq('user_id', user.id).order('transaction_date', { ascending: false }),
  ...
  const tabBtn = (t: Tab, label: string) => (
    <button onClick={() => { setTab(t); setShowForm(false) }} style={{
      background:'transparent', border:'none',
      borderBottom: tab===t ? '1px solid var(--text-lo)' : '1px solid transparent',
      padding:'8px 14px', fontSize:'11px',
      color: tab===t ? 'var(--text-hi)' : 'var(--text-muted)',
    ...
    <div style={{ maxWidth:'1000px' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'32px' }}>
        ...
        <button onClick={() => setShowForm(!showForm)}
          ...
          + {tab==='transactions' ? 'Movimiento' : 'Registro'}
        </button>
      </div>
        ...
        <>
          {showForm && (
            <div style={{ border:'1px solid var(--border)', padding:'24px', marginBottom:'28px' }}>
              ...
              <div style={{ display:'flex', gap:'12px' }}>
                ...
                <button onClick={() => { setShowForm(false); setForm(emptyTx) }} style={{ background:'transparent', border:'none', fontSize:'12px', color:'var(--text-muted)' }}>Cancelar</button>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr>{['Fecha','Tipo','Activo','Categoría','Cantidad','Precio unit.','Total','Notas'].map(h => <th key={h} style={th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {displayed.map(tx => {
      ...
      {tab === 'networth' && (
        <>
          {showForm && (
            <div style={{ border:'1px solid var(--border)', padding:'24px', marginBottom:'28px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 2fr', gap:'12px', marginBottom:'12px' }}>
                <div><label style={lbl}>Valor total del portafolio</label>
              ...
              <div style={{ display:'flex', gap:'12px' }}>
                <button onClick={saveNW} disabled={!nwForm.value||saving}
                  style={{ background:'transparent', border:'1px solid var(--text-muted)', padding:'7px 16px', fontSize:'12px', color:'var(--text-md)', opacity: !nwForm.value?0.4:1 }}>
                  {saving ? 'Guardando...' : 'Guardar'}
                </button>
                <button onClick={() => { setShowForm(false); setNwForm(emptyNW) }} style={{ background:'transparent', border:'none', fontSize:'12px', color:'var(--text-muted)' }}>Cancelar</button>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr>{['Fecha','Valor','Cambio','Notas'].map(h => <th key={h} style={th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {nwSnaps.map((snap, i) => {
