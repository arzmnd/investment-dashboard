...
const emptyTx = {
  asset_id:'', transaction_type:'buy', quantity:'', price_per_unit:'',
  total_amount:'', currency:'USD', exchange_rate:'1', notes:'',
...
export default function TransactionsPage() {
  ...
  function handleFormChange(field: string, value: string) {
    setForm(prev => {
      ...
      // Reset exchange rate when switching to USD
      if (field === 'currency' && value === 'USD') updated.exchange_rate = '1'
      return updated
    })
  }
  ...
  async function saveTx() {
    ...
    const payload: Record<string, unknown> = {
      ...
      exchange_rate: tc,
      total_usd: form.currency === 'MXN' ? total * tc : total,
  ...
  const actionBtn: React.CSSProperties = {
    ...
    padding:'7px 14px', fontSize:'12px', color:'var(--text-lo)', cursor:'pointer',
  }
        ...
        <>
            ...
            <div style={{ border:'1px solid var(--border)', padding:'24px', marginBottom:'28px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'12px' }}>
                ...
                {form.currency === 'MXN' ? (
                  <div>
                    ...
                    <input type="number" step="any" style={{ ...inp, borderColor:'var(--text-muted)' }}
                      placeholder="17.50" value={form.exchange_rate}
                      onChange={e => handleFormChange('exchange_rate', e.target.value)} />
                    {form.total_amount && form.exchange_rate && (
                      <p style={{ fontSize:'10px', color:'var(--text-muted)', marginTop:'4px', fontFamily:"'JetBrains Mono', monospace" }}>
                        = {formatCurrency(parseFloat(form.total_amount) * parseFloat(form.exchange_rate))} USD
                      </p>
                    )}
                  </div>
                ) : (
              ...
              <div style={{ display:'flex', gap:'12px' }}>
                <button onClick={saveTx} disabled={!form.total_amount || saving}
                  style={{ background:'var(--text-hi)', border:'none', padding:'8px 20px', fontSize:'12px', color:'var(--bg)', fontWeight:'600', cursor:'pointer', opacity: !form.total_amount ? 0.4 : 1 }}>
                  {saving ? 'Guardando...' : 'Guardar'}
                </button>
                <button onClick={() => { setShowForm(false); setForm(emptyTx) }}
                  style={{ background:'transparent', border:'none', fontSize:'12px', color:'var(--text-muted)', cursor:'pointer' }}>
                  Cancelar
                </button>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr>{['Fecha','Operación','Activo','Categoría','Total','USD','TC','Notas'].map(h => <th key={h} style={th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {displayed.map(tx => {
                  ...
                  return (
                    <tr key={tx.id}
                      ...
                      <td style={{ padding:'12px 16px', fontSize:'12px', color:'var(--text-hi)', borderBottom:'1px solid var(--border-sub)', fontFamily:"'JetBrains Mono', monospace" }}>
                        {tx.asset ? ((tx.asset as any).activo ?? (tx.asset as any).name) : <span style={{ color:'var(--text-faint)' }}>—</span>}
                      </td>
                      ...
                      <td style={{ padding:'12px 16px', borderBottom:'1px solid var(--border-sub)' }}>
                        ...
                        {tx.currency === 'MXN' && <span style={{ fontSize:'10px', color:'var(--text-faint)', marginLeft:'4px' }}>MXN</span>}
                      </td>
                      ...
                      <td style={{ padding:'12px 16px', fontFamily:"'JetBrains Mono', monospace", fontSize:'11px', color:'var(--text-faint)', borderBottom:'1px solid var(--border-sub)' }}>
                        {tx.currency === 'MXN' ? tx.exchange_rate : '—'}
                      </td>
            ...
            <div style={{ border:'1px solid var(--border)', padding:'24px', marginBottom:'28px' }}>
              ...
              <div style={{ display:'flex', gap:'12px' }}>
                <button onClick={saveNW} disabled={!nwForm.value || saving}
                  style={{ background:'var(--text-hi)', border:'none', padding:'8px 20px', fontSize:'12px', color:'var(--bg)', fontWeight:'600', cursor:'pointer', opacity: !nwForm.value ? 0.4 : 1 }}>
                  {saving ? 'Guardando...' : 'Guardar'}
                </button>
                <button onClick={() => { setShowForm(false); setNwForm(emptyNW) }}
                  style={{ background:'transparent', border:'none', fontSize:'12px', color:'var(--text-muted)', cursor:'pointer' }}>
                  Cancelar
                </button>
              </div>
